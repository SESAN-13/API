// document.querySelector('select').addEventListener('click',run)
//     function run(){
...
.then(res=>res.json())
.then(data=>{
   let breedArray= Object.keys(data.message) //turn the data.message into array and console log it
   for(let i=0;i<breedArray.length;i++){
       const option=document.createElement('option') //we are trying to create <option></option>
       option.value=breedArray[i]
       option.innerText=breedArray[i]  // <option value >"breed"</option>
       select.appendChild(option) // adding <option>tag to our selector
   }
})
...
const doggoInfo = {
    ...
    assignName(arr){
        this.rNames = arr[Math.floor(Math.random()*arr.length)]
        document.getElementById('Name').innerHTML=`${this.rNames}`
    ...
    yateshuffle(array) {
      ...
      assignLikes(){
          this.Likes=this.yateshuffle(this.likesList).slice(0,2)
          document.getElementById('likes').innerHTML=`Likes:${this.Likes[0]},${this.Likes[1]}`
      },

      assignDislikes(){
          this.Dislikes=this.yateshuffle(this.dislikesList).slice(0,2)
          document.getElementById('dis-like').innerHTML=`Dislikes:${this.Dislikes[0]},${this.Dislikes[1]}`
      },

      assignFunfacts(){
          this.Facts=this.factList[Math.floor(Math.random()*this.factList.length)]
          document.getElementById('Funfacts').innerHTML=`Facts:${this.Facts}`
      }
